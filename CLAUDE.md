...
- **CSP:** two separate policies, both **enforced**. `core.middleware.SecurityHeadersMiddleware` covers Django-rendered pages (admin, Swagger, ReDoc, DRF browsable API); `app/frontend/vite.config.ts` covers the SPA. `CSP_ENFORCE=False` falls back to report-only, which is the right setting while shaking out a new directive.
  ...
  - The SPA headers come from the Vite **dev server**, so they apply to `npm run dev` only; a built bundle served by anything else carries no CSP. Today the prod compose inherits `command: npm run dev`, so this is also the production policy — that inheritance is a deployment problem in its own right. Note the SPA's `script-src` still allows `'unsafe-inline'`/`'unsafe-eval'` for HMR, so enforcement here is not XSS-proof; what it buys is `frame-ancestors`, `object-src`, `base-uri`, `form-action` and a pinned `connect-src`.
